use crate::model::Language;
use std::ffi::OsStr;

#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub enum TextKey {
    AppTitle,
    HomeQuick,
    HomeKeys,
    HomeWords,
    HomeSentence,
    HomeLong,
    HomeTest,
    HomeGames,
    HomeStats,
    HomeGoals,
    HomeContent,
    HomeSettings,
    Help,
    Back,
    Confirm,
    Pause,
    Resume,
    Quit,
    Language,
    Difficulty,
    Duration,
    WordCount,
    Speed,
    Accuracy,
    Errors,
    Backspaces,
    Remaining,
    Progress,
    Input,
    Prompt,
    Result,
    Previous,
    Best,
    PersonalBest,
    PersonalBestUpdated,
    GoalMet,
    GoalMissed,
    History,
    WeakKeys,
    WeakKeysPerfect,
    Streak,
    DailyMinutes,
    Theme,
    Sources,
    License,
    UpdateAvailable,
    UpdateLater,
    UpdateSkip,
    UpdateCommand,
    PasteIgnored,
    TooSmall,
    SaveFailed,
    CorruptFile,
    NoData,
    TestGrade,
    Source,
    Stop,
    Preset,
    Stage,
    Random,
    RepeatWeakKeys,
    Start,
    Time,
    Items,
    Words,
    Quote,
    Text,
    Easy,
    Medium,
    Hard,
    Mixed,
    Title,
    Author,
    Tags,
    WordRain,
    Score,
    Level,
    Combo,
    Cleared,
    MaxCombo,
    MissedWord,
    Target,
    CollisionLine,
    CorrectionNeeded,
    Retry,
    GameOver,
    LeaveGameConfirm,
    NoPlayableWords,
    BossBattle,
    IronWarden,
    ThornQueen,
    NullArchon,
    PrismSeraph,
    BossLocked,
    DifficultyLocked,
    Victory,
    Defeat,
    BossUnlocked,
    DifficultyUnlocked,
    Hearts,
    Phase,
    Checksum,
    InputLocked,
}

impl TextKey {
    pub const ALL: &'static [Self] = &[
        Self::AppTitle,
        Self::HomeQuick,
        Self::HomeKeys,
        Self::HomeWords,
        Self::HomeSentence,
        Self::HomeLong,
        Self::HomeTest,
        Self::HomeGames,
        Self::HomeStats,
        Self::HomeGoals,
        Self::HomeContent,
        Self::HomeSettings,
        Self::Help,
        Self::Back,
        Self::Confirm,
        Self::Pause,
        Self::Resume,
        Self::Quit,
        Self::Language,
        Self::Difficulty,
        Self::Duration,
        Self::WordCount,
        Self::Speed,
        Self::Accuracy,
        Self::Errors,
        Self::Backspaces,
        Self::Remaining,
        Self::Progress,
        Self::Input,
        Self::Prompt,
        Self::Result,
        Self::Previous,
        Self::Best,
        Self::PersonalBest,
        Self::PersonalBestUpdated,
        Self::GoalMet,
        Self::GoalMissed,
        Self::History,
        Self::WeakKeys,
        Self::WeakKeysPerfect,
        Self::Streak,
        Self::DailyMinutes,
        Self::Theme,
        Self::Sources,
        Self::License,
        Self::UpdateAvailable,
        Self::UpdateLater,
        Self::UpdateSkip,
        Self::UpdateCommand,
        Self::PasteIgnored,
        Self::TooSmall,
        Self::SaveFailed,
        Self::CorruptFile,
        Self::NoData,
        Self::TestGrade,
        Self::Source,
        Self::Stop,
        Self::Preset,
        Self::Stage,
        Self::Random,
        Self::RepeatWeakKeys,
        Self::Start,
        Self::Time,
        Self::Items,
        Self::Words,
        Self::Quote,
        Self::Text,
        Self::Easy,
        Self::Medium,
        Self::Hard,
        Self::Mixed,
        Self::Title,
        Self::Author,
        Self::Tags,
        Self::WordRain,
        Self::Score,
        Self::Level,
        Self::Combo,
        Self::Cleared,
        Self::MaxCombo,
        Self::MissedWord,
        Self::Target,
        Self::CollisionLine,
        Self::CorrectionNeeded,
        Self::Retry,
        Self::GameOver,
        Self::LeaveGameConfirm,
        Self::NoPlayableWords,
        Self::BossBattle,
        Self::IronWarden,
        Self::ThornQueen,
        Self::NullArchon,
        Self::PrismSeraph,
        Self::BossLocked,
        Self::DifficultyLocked,
        Self::Victory,
        Self::Defeat,
        Self::BossUnlocked,
        Self::DifficultyUnlocked,
        Self::Hearts,
        Self::Phase,
        Self::Checksum,
        Self::InputLocked,
    ];
}

pub const fn text(language: Language, key: TextKey) -> &'static str {
    match language {
        Language::Ko => match key {
            TextKey::AppTitle => "Typerlude",
            TextKey::HomeQuick => "빠른 연습",
            TextKey::HomeKeys => "키 연습",
            TextKey::HomeWords => "단어 연습",
            TextKey::HomeSentence => "문장 연습",
            TextKey::HomeLong => "긴 글 연습",
            TextKey::HomeTest => "타자 시험",
            TextKey::HomeGames => "게임",
            TextKey::HomeStats => "통계",
            TextKey::HomeGoals => "목표",
            TextKey::HomeContent => "콘텐츠",
            TextKey::HomeSettings => "설정",
            TextKey::Help => "도움말",
            TextKey::Back => "뒤로",
            TextKey::Confirm => "확인",
            TextKey::Pause => "일시 정지",
            TextKey::Resume => "계속",
            TextKey::Quit => "종료",
            TextKey::Language => "언어",
            TextKey::Difficulty => "난이도",
            TextKey::Duration => "시간",
            TextKey::WordCount => "단어 수",
            TextKey::Speed => "속도",
            TextKey::Accuracy => "정확도",
            TextKey::Errors => "오류",
            TextKey::Backspaces => "백스페이스",
            TextKey::Remaining => "남은 시간",
            TextKey::Progress => "진행",
            TextKey::Input => "현재 입력",
            TextKey::Prompt => "원문",
            TextKey::Result => "결과",
            TextKey::Previous => "이전",
            TextKey::Best => "최고",
            TextKey::PersonalBest => "개인 최고 기록",
            TextKey::PersonalBestUpdated => "개인 최고 기록 갱신!",
            TextKey::GoalMet => "목표 달성",
            TextKey::GoalMissed => "목표 미달",
            TextKey::History => "기록",
            TextKey::WeakKeys => "약한 키",
            TextKey::WeakKeysPerfect => "약한 키 없음 · 분석된 모든 키 정확도 100%",
            TextKey::Streak => "연속 기록",
            TextKey::DailyMinutes => "하루 연습 시간",
            TextKey::Theme => "테마",
            TextKey::Sources => "출처",
            TextKey::License => "라이선스",
            TextKey::UpdateAvailable => "업데이트 가능",
            TextKey::UpdateLater => "나중에",
            TextKey::UpdateSkip => "이번 버전 건너뛰기",
            TextKey::UpdateCommand => "업데이트 명령",
            TextKey::PasteIgnored => "붙여넣기 무시됨",
            TextKey::TooSmall => "터미널이 너무 작습니다",
            TextKey::SaveFailed => "저장 실패",
            TextKey::CorruptFile => "손상된 파일",
            TextKey::NoData => "데이터 없음",
            TextKey::TestGrade => "시험 등급",
            TextKey::Source => "출처",
            TextKey::Stop => "종료 기준",
            TextKey::Preset => "사전 설정",
            TextKey::Stage => "단계",
            TextKey::Random => "무작위",
            TextKey::RepeatWeakKeys => "약한 키 반복",
            TextKey::Start => "시작",
            TextKey::Time => "시간",
            TextKey::Items => "항목",
            TextKey::Words => "단어",
            TextKey::Quote => "인용문",
            TextKey::Text => "글",
            TextKey::Easy => "쉬움",
            TextKey::Medium => "보통",
            TextKey::Hard => "어려움",
            TextKey::Mixed => "혼합",
            TextKey::Title => "제목",
            TextKey::Author => "저자",
            TextKey::Tags => "태그",
            TextKey::WordRain => "단어비",
            TextKey::Score => "점수",
            TextKey::Level => "레벨",
            TextKey::Combo => "콤보",
            TextKey::Cleared => "제거한 단어",
            TextKey::MaxCombo => "최대 콤보",
            TextKey::MissedWord => "놓친 단어",
            TextKey::Target => "대상",
            TextKey::CollisionLine => "놓치면 종료",
            TextKey::CorrectionNeeded => "수정 필요",
            TextKey::Retry => "다시 하기",
            TextKey::GameOver => "게임 종료",
            TextKey::LeaveGameConfirm => "q를 한 번 더 누르면 게임을 나갑니다",
            TextKey::NoPlayableWords => "플레이 가능한 단어가 없습니다",
            TextKey::BossBattle => "보스 배틀",
            TextKey::IronWarden => "IRON WARDEN",
            TextKey::ThornQueen => "THORN QUEEN",
            TextKey::NullArchon => "NULL ARCHON",
            TextKey::PrismSeraph => "PRISM SERAPH",
            TextKey::BossLocked => "잠긴 보스",
            TextKey::DifficultyLocked => "잠긴 난이도",
            TextKey::Victory => "승리",
            TextKey::Defeat => "패배",
            TextKey::BossUnlocked => "새 보스 해금",
            TextKey::DifficultyUnlocked => "새 난이도 해금",
            TextKey::Hearts => "체력",
            TextKey::Phase => "페이즈",
            TextKey::Checksum => "체크섬",
            TextKey::InputLocked => "입력 잠김",
        },
        Language::En => match key {
            TextKey::AppTitle => "Typerlude",
            TextKey::HomeQuick => "Quick practice",
            TextKey::HomeKeys => "Key practice",
            TextKey::HomeWords => "Word practice",
            TextKey::HomeSentence => "Sentence practice",
            TextKey::HomeLong => "Long-text practice",
            TextKey::HomeTest => "Typing test",
            TextKey::HomeGames => "Games",
            TextKey::HomeStats => "Statistics",
            TextKey::HomeGoals => "Goals",
            TextKey::HomeContent => "Content",
            TextKey::HomeSettings => "Settings",
            TextKey::Help => "Help",
            TextKey::Back => "Back",
            TextKey::Confirm => "Confirm",
            TextKey::Pause => "Pause",
            TextKey::Resume => "Resume",
            TextKey::Quit => "Quit",
            TextKey::Language => "Language",
            TextKey::Difficulty => "Difficulty",
            TextKey::Duration => "Duration",
            TextKey::WordCount => "Word count",
            TextKey::Speed => "Speed",
            TextKey::Accuracy => "Accuracy",
            TextKey::Errors => "Errors",
            TextKey::Backspaces => "Backspaces",
            TextKey::Remaining => "Remaining",
            TextKey::Progress => "Progress",
            TextKey::Input => "Input",
            TextKey::Prompt => "Prompt",
            TextKey::Result => "Result",
            TextKey::Previous => "Previous",
            TextKey::Best => "Best",
            TextKey::PersonalBest => "Personal best",
            TextKey::PersonalBestUpdated => "Personal best updated!",
            TextKey::GoalMet => "Goal met",
            TextKey::GoalMissed => "Goal missed",
            TextKey::History => "History",
            TextKey::WeakKeys => "Weak keys",
            TextKey::WeakKeysPerfect => "No weak keys · All analyzed keys are 100% accurate",
            TextKey::Streak => "Streak",
            TextKey::DailyMinutes => "Daily minutes",
            TextKey::Theme => "Theme",
            TextKey::Sources => "Sources",
            TextKey::License => "License",
            TextKey::UpdateAvailable => "Update available",
            TextKey::UpdateLater => "Later",
            TextKey::UpdateSkip => "Skip this version",
            TextKey::UpdateCommand => "Update command",
            TextKey::PasteIgnored => "Paste ignored",
            TextKey::TooSmall => "Terminal is too small",
            TextKey::SaveFailed => "Save failed",
            TextKey::CorruptFile => "Corrupt file",
            TextKey::NoData => "No data",
            TextKey::TestGrade => "Test grade",
            TextKey::Source => "Source",
            TextKey::Stop => "Stop",
            TextKey::Preset => "Preset",
            TextKey::Stage => "Stage",
            TextKey::Random => "Random",
            TextKey::RepeatWeakKeys => "Repeat weak keys",
            TextKey::Start => "Start",
            TextKey::Time => "Time",
            TextKey::Items => "Items",
            TextKey::Words => "Words",
            TextKey::Quote => "Quote",
            TextKey::Text => "Text",
            TextKey::Easy => "Easy",
            TextKey::Medium => "Medium",
            TextKey::Hard => "Hard",
            TextKey::Mixed => "Mixed",
            TextKey::Title => "Title",
            TextKey::Author => "Author",
            TextKey::Tags => "Tags",
            TextKey::WordRain => "Word Rain",
            TextKey::Score => "Score",
            TextKey::Level => "Level",
            TextKey::Combo => "Combo",
            TextKey::Cleared => "Cleared words",
            TextKey::MaxCombo => "Max combo",
            TextKey::MissedWord => "Missed word",
            TextKey::Target => "Target",
            TextKey::CollisionLine => "Miss line",
            TextKey::CorrectionNeeded => "Correction needed",
            TextKey::Retry => "Retry",
            TextKey::GameOver => "Game over",
            TextKey::LeaveGameConfirm => "Press q again to leave the game",
            TextKey::NoPlayableWords => "No playable words",
            TextKey::BossBattle => "Boss Battle",
            TextKey::IronWarden => "IRON WARDEN",
            TextKey::ThornQueen => "THORN QUEEN",
            TextKey::NullArchon => "NULL ARCHON",
            TextKey::PrismSeraph => "PRISM SERAPH",
            TextKey::BossLocked => "Boss locked",
            TextKey::DifficultyLocked => "Difficulty locked",
            TextKey::Victory => "Victory",
            TextKey::Defeat => "Defeat",
            TextKey::BossUnlocked => "Boss unlocked",
            TextKey::DifficultyUnlocked => "Difficulty unlocked",
            TextKey::Hearts => "Hearts",
            TextKey::Phase => "Phase",
            TextKey::Checksum => "Checksum",
            TextKey::InputLocked => "Input locked",
        },
    }
}

pub fn initial_ui_language(lc_all: Option<&str>, lang: Option<&str>) -> Language {
    initial_ui_language_os(lc_all.map(OsStr::new), lang.map(OsStr::new))
}

pub(crate) const fn result_actions(language: Language, can_start_next: bool) -> &'static str {
    match (language, can_start_next) {
        (Language::Ko, true) => "r/ㄱ: 다시 연습 · n: 다음 · Esc: 메뉴",
        (Language::Ko, false) => "r/ㄱ: 다시 연습 · Esc: 메뉴",
        (Language::En, true) => "r/ㄱ: Retry · n: Next · Esc: Menu",
        (Language::En, false) => "r/ㄱ: Retry · Esc: Menu",
    }
}

pub(crate) fn initial_ui_language_os(lc_all: Option<&OsStr>, lang: Option<&OsStr>) -> Language {
    let locale = lc_all.or(lang).and_then(OsStr::to_str).unwrap_or_default();
    let prefix = locale.split(['_', '-', '.']).next().unwrap_or_default();
    if prefix.eq_ignore_ascii_case("ko") {
        Language::Ko
    } else {
        Language::En
    }
}

#[cfg(test)]
mod locale_tests {
    use super::initial_ui_language_os;
    use crate::model::Language;

    #[cfg(unix)]
    #[test]
    fn present_non_unicode_lc_all_wins_over_korean_lang_as_english() {
        use std::{
            ffi::{OsStr, OsString},
            os::unix::ffi::OsStringExt,
        };

        let non_unicode = OsString::from_vec(vec![0xff]);
        assert_eq!(
            initial_ui_language_os(
                Some(non_unicode.as_os_str()),
                Some(OsStr::new("ko_KR.UTF-8")),
            ),
            Language::En
        );
    }

    #[cfg(windows)]
    #[test]
    fn present_non_unicode_lc_all_wins_over_korean_lang_as_english() {
        use std::{
            ffi::{OsStr, OsString},
            os::windows::ffi::OsStringExt,
        };

        let non_unicode = OsString::from_wide(&[0xd800]);
        assert_eq!(
            initial_ui_language_os(
                Some(non_unicode.as_os_str()),
                Some(OsStr::new("ko_KR.UTF-8")),
            ),
            Language::En
        );
    }
}
