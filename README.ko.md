...
한국어 또는 영어와 쉬움·보통·어려움을 골라 끝없이 플레이합니다. 단어 10개를 없앨 때마다
더 빨라지며, 단어 하나라도 놓치면 즉시 끝납니다. 오타는 입력창에 남으므로 `Backspace`로
고칠 수 있고, 전부 지우면 다른 단어를 선택할 수 있습니다. `Esc`로 일시 정지하고, 정지 중
`q` 또는 `ㅂ`을 두 번 누르면 나가며, 결과 화면에서 `Enter`로 다시 시작합니다.

### 보스전

하트 3개로 보스 3명 중 하나와 활성 시간 90초 동안 싸웁니다. 보스마다 고유한 입력 패턴과
2페이즈 스킬이 있습니다. 각 보스의 난이도는 쉬움 → 보통 → 어려움 순서로 열리며, 쉬움을
클리어하면 다음 보스도 열립니다. 빈 별과 채운 별로 가장 높이 클리어한 난이도를 표시합니다.
...
게임 결과는 세션 기록에 추가하지 않습니다. 단어비 최고 점수는 언어·난이도별로 저장합니다.
보스전 클리어 별은 언어가 달라도 공유하며, 최고 점수는 보스·언어·난이도별로 저장합니다.
